...
var carrosFord = carros.filter(function(car) {
  return car.marca === 'ford';
});
...
var s = carros.some(function(car) {
  return car.marca === 'ford';
});
s //true
...
carros.map(function(car) {
  return car.marca;
}); // ['ford', 'chevrolet', 'fiat']

//Mapeia quantos caracteres tem a marca de cada elemento do array
var mapLength = carros.map(function(car) {
  return car.marca.length;
});
mapLength // [4 ,9, 4]
...
}, 0); //98000 || prev -> elemento anterior + cur -> elemento atual, 0 -> começa do elemento 0
